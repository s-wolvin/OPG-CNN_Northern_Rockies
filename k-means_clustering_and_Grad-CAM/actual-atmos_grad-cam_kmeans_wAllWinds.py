...
class weighted_MSE(tf_k.losses.Loss):
    def call(self, y_true, y_pred):
        # formulate weights
        # mean_val = tf.reduce_mean(tf.abs(y_true))
        # weights = (tf.abs(y_true) / (mean_val + 1e-6))  
        
        # formulate squared error
        se  = tf.math.square(y_pred - y_true)
        
        # apply weights
        # weight_se = weights * se
...
def grad_cam(atmos_clust):
    ...
    heatmap_daily = np.max(heatmap, axis=3) # create max of all between facets per day
    
    heatmap = np.mean(heatmap_daily, axis=0) # create mean of all days

    # composite max
    comp_max = np.max(heatmap[:])

    heatmap = heatmap / comp_max  # Scale to max of 1
    heatmap[heatmap < 0] = 0  # Remove all the negative values
    
    # scale daily the same way as the mean
    heatmap_daily = heatmap_daily / np.max(heatmap_daily, axis = (1,2))[:, np.newaxis, np.newaxis]
    heatmap_daily[heatmap_daily < 0] = 0 
    
    return heatmap, heatmap_daily
...
for clust in np.unique(kmeans['cluster'].values):
    ...
    grad_cam_clust, _ = grad_cam(atmos_clust)
    ...
    q = ax[0].quiver(loni[::10, ::15], lati[::10, ::15], atmos_clust_mean.uwnd700.values[::10, ::15], 
                  ...
                  scale=uwnd_scl, pivot='mid', transform=datacrs, zorder=7,
                  color=uwnd_col, headwidth=6)
    ax[0].quiverkey(q, X=0.285, Y=0.26, U=15, label='15 m s$^{-1}$', labelpos='E', 
    ...
    if clust+1 == 1:
        hatch = ax[0].contourf(h_lon, h_lat, grad_cam_clust, colors='none', zorder=8, 
                                transform=datacrs, hatches=[None, None, None, None, None, None, None, None, None, 'oo'], 
    ...
    ax[1].quiverkey(q, X=0.54, Y=0.26, U=5, label='5 m s$^{-1}$', labelpos='E', 
                    coordinates='figure', fontproperties={'size':lbl_sz})
...
#%% Formulate overlap between region and atmos variables

# for clust in np.unique(kmeans['cluster'].values):
for clust in [0, 1, 3, 4]:
    ...
    grad_cam_clust, grad_cam_daily = grad_cam(atmos_clust)    
    ...
    grad_cam_daily = xr.Dataset(data_vars=dict(gradcam =(['time', 'lat', 'lon'], grad_cam_daily)), 
                                coords=dict(time= atmos_clust.time, lat=h_lat[:,0], lon=h_lon[0,:]))

    # interpolate to atmos grid
    grad_cam_daily = grad_cam_daily.interp(lat = atmos_clust.lat, lon = atmos_clust.lon)

    # formulate true values of the atmosphere
    true_atmos = (atmos * atmos_std) + atmos_mean

    # Subset to northern rockies region
    nr_gradcam = grad_cam_daily.sel(lat = slice(rgn_bnds[0], rgn_bnds[1]), 
                                    lon = slice(rgn_bnds[2], rgn_bnds[3]))
    print("Northern Rockies")
    print(np.sum(nr_gradcam.max(dim = ['lat', 'lon']) > 0.8)/len(nr_gradcam.time.values))
    
    # Subset to the columbia river gorge
    crg_gradcam = grad_cam_daily.sel(lat = slice(45, 47), 
                                    lon = slice(-124, -117))
    print("Columbia river gorge")
    print(np.sum(crg_gradcam.max(dim = ['lat', 'lon']) > 0.8)/len(crg_gradcam.time.values))
    
    # Subset to norhtern california
    nc_gradcam = grad_cam_daily.sel(lat = slice(39, 42), 
                                    lon = slice(-124, -120))
    print("Northern california")
    print(np.sum(nc_gradcam.max(dim = ['lat', 'lon']) > 0.8)/len(nc_gradcam.time.values))
    
    # Subset to southern california
    sc_gradcam = grad_cam_daily.sel(lat = slice(33, 36.5), 
                                    lon = slice(-118, -115))
    print("Southern california")
    print(np.sum(sc_gradcam.max(dim = ['lat', 'lon']) > 0.8)/len(sc_gradcam.time.values))
    
    # Percent of days with gradcam greater than 0.9 and IVT > 250
    ivt = true_atmos.IVTsfc.sel(time = atmos_clust.time)
    ar_gradcam = grad_cam_daily.where(ivt.values > 250, 0)
    print("IVT")
    print(np.sum(ar_gradcam.max(dim=['lat', 'lon']) > 0.8)/np.sum(ivt.max(dim=['lat','lon']) > 250))
    
    # Percent of days with gradcam greater than 0.9 and IVT > 250
    ivt = atmos.IVTsfc.sel(time = atmos_clust.time)
    ar_gradcam = grad_cam_daily.where(ivt.values > 2, 0)
    print("IVT")
    print(np.sum(ar_gradcam.max(dim=['lat', 'lon']) > 0.8)/np.sum(ivt.max(dim=['lat','lon']) > 2))
    
    # Percent of days with gradcam greater than 0.9 and Q anom > 0.5
    q_anom = atmos.shum850.sel(time = atmos_clust.time)
    qanom_gradcam = grad_cam_daily.where(q_anom.values > 2, 0)
    print("Specific humidity")
    print(np.sum(qanom_gradcam.max(dim=['lat', 'lon']) > 0.8)/np.sum(q_anom.max(dim=['lat','lon']) > 2))
    
    # Percent of days with gradcam greater than 0.9 and w-winds > 0.5
    wwnd700 = atmos.wwnd700.sel(time = atmos_clust.time)
    wanom_gradcam = grad_cam_daily.where(wwnd700.values > 2, 0)
    print("W-winds")
    print(np.sum(wanom_gradcam.max(dim=['lat', 'lon']) > 0.8)/np.sum(wwnd700.max(dim=['lat','lon']) > 2))

    
    # Percent of days with gradcam greater than 0.9 and u-winds > 0.5
    uwnd700 = atmos.uwnd700.sel(time = atmos_clust.time)
    uanom_gradcam = grad_cam_daily.where(uwnd700.values > 2, 0)
    print("U-winds")
    print(np.sum(uanom_gradcam.max(dim=['lat', 'lon']) > 0.8)/np.sum(uwnd700.max(dim=['lat','lon']) > 2))

    # Percent of days with gradcam greater than 0.9 and w-winds > 0.5
    precipsfc = atmos.precipsfc.sel(time = atmos_clust.time)
    precipsfc_gradcam = grad_cam_daily.where(precipsfc.values > 2, 0)
    print("Precpitation")
    print(np.sum(precipsfc_gradcam.max(dim=['lat', 'lon']) > 0.8)/np.sum(precipsfc.max(dim=['lat','lon']) > 2))

    # Percent of days with gradcam greater than 0.9 and w-winds > 0.5
    precipsfc = true_atmos.precipsfc.sel(time = atmos_clust.time)
    precipsfc_gradcam = grad_cam_daily.where(precipsfc.values > 0.01, 0)
    print("True Precpitation")
    print(np.sum(precipsfc_gradcam.max(dim=['lat', 'lon']) > 0.8)/np.sum(precipsfc.max(dim=['lat','lon']) > 0.01))
